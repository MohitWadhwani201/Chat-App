...
const MessageContainer = () => {
	...
	const renderMesages = () => {
		...
		return selectedChatMessages.map((message, index) => {
			const messageDate = moment(message, message.timestamp).format("YYYY-MM-DD");
	...
	return (
		<div className="flex-1 overflow-y-auto scrollbar-hiddem p-4 px-8 md:w-[65vw] lg:w-[70vw] xl:w-65vw] w-full">
			{renderMesages()}
			{/* {rederMesages().length > 0 ? (
			rederMesages().map((message, index) => (
